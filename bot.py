...
bot_token = '1920340707:AAFL2FRti6oMTpenhOBx_sQQ-h9F8cjwk7s'
api_id = 5582493
api_hash = '16daf25dbeb9f44aa4248bd26f991295'
...
async def down_to_tel(url,bot,ev,msg):
    try:
         ...
         if req.status_code == 200:
            ...
            mult_file =  multiFile.MultiFile(get_name(file_name)+'.7z',1024 * 1024 * 1800)
            zip = ZipFile(mult_file,  mode='w', compression=zipfile.ZIP_DEFLATED)
            zip.write(file_name)
            zip.close()
            mult_file.close()
...
async def process_txt(file,bot,ev,msg):
    ...
    if len(file) > 0:
        mult_file =  multiFile.MultiFile(get_name(baseName)+'.7z',1024 * 1024 * 1800)
